 ...
 * hardTimeout). This hard deadline may be decreased if a subsequent log event has an
 * associated hard timeout that would result in an earlier hard deadline.
 * <li><b>soft timeouts</b> - these timeouts cause a soft deadline to be set for when flushing
 * should occur. Unlike the hard deadline, this deadline may be increased if a subsequent log
 * event occurs before any deadline is reached. Note however, that the timeout used to
 * calculate the soft deadline is set to the minimum of the timeouts associated with the log
 * events that have occurred before the deadline. E.g., if a log event associated with a 5s
 * soft timeout occurs, and then is followed by a log event associated with a 10s soft
 * timeout, the soft deadline will be set as if the second log event had a had 5s soft
 * timeout.
 * </ul>
 ...
 * log events.
 * <p>
 * This class also allows logs to be collected while the JVM is shutting down. This can be
 * enabled by setting closeOnShutdown to false. When the JVM starts shutting down, the appender will
 ...
 * the shutdown is allowed to continue. It is soft in the sense that if a log event occurs
 * before this delay expires, the delay is reset based on the current time.
...
public abstract class AbstractBufferedRollingFileAppender extends AbstractAppender
     ...
     * down
     * and the appender will do its best to sync those logs before the JVM shuts down. This presents
     * a
     * tradeoff between capturing more log events and potential data loss if the log events cannot
     * be
     * flushed and synced before the JVM is killed.
     *
     ...
     * sync).
     * Care should be taken to ensure this period does not significantly differ from the lowest
     * timeout since that will cause undue delay from when a timeout expires and when a flush
     * occurs.
     *
     ...
     * base class needs to perform actions before/after the derived class' {@link
     * #activateOptionsHook(long)} method.
     */
    @Override
     ...
     * This method is {@code final} to ensure it is not overridden by derived classes since this
     * base class needs to perform actions before/after the derived class' {@link
     * #appendHook(LogEvent)} method. This method is also marked {@code synchronized} since it can
     * be
     * called from multiple logging threads.
     *
        ...
        public void run() {
            ...
            while (true) {
                try {
                    ...
                    LOGGER.debug(
                            "Received interrupt message for graceful shutdown of BackgroundFlushThread"
                    );

                    delayedShutdownRequested = closeWithDelayedShutdown;
                    if (closeOnShutdown || !delayedShutdownRequested) {
                        break;
                    }
                    long currentTimestamp = timeSource.getCurrentTimeInMilliseconds();
                    shutdownSoftTimeoutTimestamp = currentTimestamp + shutdownSoftTimeout;
    ...
    /**
     * Thread to synchronize log files in the background (by calling {@link #sync(String, long,
     * boolean, Map<String, Object>) sync}). The thread maintains a request queue that callers
     * should
     * populate.
     */
    private class BackgroundSyncRunnable implements Runnable {
        ...
        public void addShutdownRequest() {
            ...
            while (!requests.offer(shutdownRequest)) {}
        }
        ...
        public void addSyncRequest(
            ...
            while (!requests.offer(syncRequest)) {}
        }
     ...
     * occur during shutdown can be collected.
     */
    ...
    protected static class Builder<B extends Builder<B>> extends AbstractAppender.Builder<B> {
         ...
         *
         * @param flushHardTimeoutsInMinutes A CSV string of kv-pairs. The key being the log-level
         * in
         ...
         * be assigned the timeout of the INFO level.
         *
         * @param flushSoftTimeoutsInSeconds A CSV string of kv-pairs. The key being the log-level
         * in
        ...
        /** @param shutdownSoftTimeoutInMilliseconds The soft shutdown timeout in milliseconds */
